...
class TelegramSource(MessageSource):
    """
    Telegram message listener adapter using Telethon.
    Listens to configured source channels/groups, extracts text, media,
    and metadata, then enqueues raw events for processing.
    """
...
async def run_telegram_listener():
    ...
    source = TelegramSource(settings=settings)
    try:
        await source.listen()
